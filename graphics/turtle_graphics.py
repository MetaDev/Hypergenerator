...
def prod_f(pos,angle,n):
    pos1 = pos + np.array((d * np.cos(np.deg2rad(angle)),d * np.sin(np.deg2rad(angle))))
    lines.append(pos);lines.append(pos1)
    return (pos1,angle,n)
productions ={
...
def decode(string,pos,angle,n):
    for c in string:
        ...
        elif c=='[':    stack.append((pos,angle,n))
        elif c==']':    (pos,angle,n)=stack.pop()
        elif n > 0:
...
def draw_line(pos,pos1,angle,width):
    scene.visuals.Line(pos=np.array((pos,pos1)), color=(1,1,0,1), parent=viewbox.scene)
...
#draw lines
scene.visuals.Line(pos=np.array(lines), color=(1,1,0,1),connect='segments', parent=viewbox.scene)
